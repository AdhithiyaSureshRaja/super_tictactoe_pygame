...
game_over = False
winning_line = None  # ((start_x, start_y), (end_x, end_y), color)
...
def check_small_winner(board, br, bc):
    # Check rows, columns, diagonals
    for i in range(3):
        if board[i][0] == board[i][1] == board[i][2] != 0:
            return board[i][0], ((bc*3*CELL_SIZE, br*3*CELL_SIZE + i*CELL_SIZE + CELL_SIZE//2),
                                  (bc*3*CELL_SIZE + 3*CELL_SIZE, br*3*CELL_SIZE + i*CELL_SIZE + CELL_SIZE//2))
        if board[0][i] == board[1][i] == board[2][i] != 0:
            return board[0][i], ((bc*3*CELL_SIZE + i*CELL_SIZE + CELL_SIZE//2, br*3*CELL_SIZE),
                                  (bc*3*CELL_SIZE + i*CELL_SIZE + CELL_SIZE//2, br*3*CELL_SIZE + 3*CELL_SIZE))
    if board[0][0] == board[1][1] == board[2][2] != 0:
        return board[0][0], ((bc*3*CELL_SIZE, br*3*CELL_SIZE),
                              (bc*3*CELL_SIZE + 3*CELL_SIZE, br*3*CELL_SIZE + 3*CELL_SIZE))
    if board[0][2] == board[1][1] == board[2][0] != 0:
        return board[0][2], ((bc*3*CELL_SIZE + 3*CELL_SIZE, br*3*CELL_SIZE),
                              (bc*3*CELL_SIZE, br*3*CELL_SIZE + 3*CELL_SIZE))
    return 0, None
...
def check_big_winner():
    ...
    for i in range(3):
        if big_winner[i][0] == big_winner[i][1] == big_winner[i][2] != 0:
            return big_winner[i][0], ((0, i*3*CELL_SIZE + 1.5*CELL_SIZE), (WIDTH, i*3*CELL_SIZE + 1.5*CELL_SIZE))
        if big_winner[0][i] == big_winner[1][i] == big_winner[2][i] != 0:
            return big_winner[0][i], ((i*3*CELL_SIZE + 1.5*CELL_SIZE, 0), (i*3*CELL_SIZE + 1.5*CELL_SIZE, HEIGHT))
    if big_winner[0][0] == big_winner[1][1] == big_winner[2][2] != 0:
        return big_winner[0][0], ((0, 0), (WIDTH, HEIGHT))
    if big_winner[0][2] == big_winner[1][1] == big_winner[2][0] != 0:
        return big_winner[0][2], ((WIDTH, 0), (0, HEIGHT))
    return 0, None

def reset_game():
    global big_board, big_winner, current_player, next_board, game_over, winning_line
...
while running:
    ...
    if game_over and winning_line:
        ...
        screen.blit(text, (WIDTH//6, HEIGHT//2 - 30))
    pygame.display.flip()
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and not game_over:
            ...
            if (next_board is None or next_board == (br, bc)) and big_board[small_index][sr][sc] == 0 and big_winner[br][bc] == 0:
                ...
                # When checking small board winner
                w, line = check_small_winner(big_board[small_index], br, bc)
                if w != 0:
                    big_winner[br][bc] = w
                    # Ensure winning_line has color
                    winning_line = (*line, RED if w == 1 else BLUE)

                # When checking big board winner
                w_big, line_big = check_big_winner()
                if w_big != 0:
                    winner = w_big
                    winning_line = (*line_big, RED if w_big == 1 else BLUE)
                    game_over = True
